...
module KVSAlg3 = 
 struct 
  ...
  let coq_StateRec_rect f s =
    let { store = x; coq_rec = x0; dep = x1 } = s in f x x0 x1
  ...
  let coq_UpdateRec_rect f u =
    let { sender_node = x; sender_dep = x0 } = u in f x x0
  ...
  let init_method init_val =
    state (fun k -> entry init_val 0 0) (fun n -> 0) (fun n -> 0)
  ...
  let put_method n this k v =
    ...
    let r = this.coq_rec in
    let d = this.dep in
    let d' = SysPredefs.override d n (plus (r n) (Pervasives.succ 0)) in
    let r' = SysPredefs.override r n (plus (r n) (Pervasives.succ 0)) in
  ...
  let guard_method n this k v u =
    ...
    (&&)
      ((fun a b c -> List.fold_left a c b) (fun b n0 ->
        (&&) b ((<=) (d' n0) (r n0))) SysPredefs.nids true)
      ((=) (d' n') (plus (r n') (Pervasives.succ 0)))
  ...
  let update_method n this k v u =
    ...
    let d'' = fun n0 -> Pervasives.max (d n0) (d' n0) in
